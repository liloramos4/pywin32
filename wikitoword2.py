...
if respuesta.lower() == 'si':
    def todaslaspaginas():
        ...
        def extract_pages_recursive(page, level=1):
            ...
            # Reemplazar <texto lo que sea> con (texto lo que sea)
            content = re.sub(r'<(.*?)>', r'(\\1)', content)
            # quitar mayusculas formatos de imagen
            content = re.sub(r'\\.(PNG|JPG|JPEG|GIF)', lambda x: x.group().lower(), content)
            # Reemplazar '(/code)' con ''
            content = re.sub(r'\(/code\\)', '', content)
            # quitar medidax después del enlace imagen
            content = re.sub(r'=\\d+x\\d*\\s*\\)', ')', content)
            # quitar medidax después del enlace imagen
            content = re.sub(r'(png|jpg|jpeg) \\)', r'\\1)', content)
...
while True:
    ...
    for index, para in enumerate(doc.Paragraphs):
        ...
        # Ignora las líneas que comienzan con una tubería
...
def apply_formatting(doc, text_list, color_code, color_name):
    print(f"Entering apply_formatting with color_name = {color_name}")
    for para in doc.Paragraphs:
