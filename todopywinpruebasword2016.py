...
link_patterns = [
    re.compile(r'\[.*?\]\(.*?\)'),  # Enlace tipo1
    re.compile(r'- \[.*?\]\(.*?\)'),  # Enlace tipo2
    re.compile(r'- https://.*'),  # Enlace tipo3
    re.compile(r'!\[.*?\]\(https://.*\)')  # Enlace tipo4
]
...
while True:
    ...
    for index, para in enumerate(doc.Paragraphs):
        if "|" in para.Range.Text:
            if start_table is None:
                start_table = index
            table_lines.append(para.Range.Text.strip())
        elif start_table is not None:
            end_table = index
            break

    if not table_lines:  # Si no se encuentra ninguna tabla, se detiene el bucle
        break

    # Procesamiento de la tabla en markdown
    data = []
    headers = table_lines.pop(1)  # Remover la línea de separación
    for line in table_lines:
        cells = [cell.strip() for cell in line.split('|')[1:-1]]
        data.append(cells)
    
    # Borrar el rango de la tabla en markdown
    counter = 0
    for para in doc.Paragraphs:
        counter += 1
    ...
    # Agregar la tabla a Word en el lugar donde estaba la tabla en markdown
    ...
    for i, row_data in enumerate(data):
        for j, cell_data in enumerate(row_data):
            ...
            cell.Range.Text = cell_data.strip()

    # Aplicar el estilo "Acc_Table_1"
...
# Recorrer todas las páginas del documento en orden inverso
for i in range(doc.ComputeStatistics(win32.constants.wdStatisticPages), 0, -1):
    # Obtener el rango de la página actual
    page_range = doc.Bookmarks(f"\\Page").Range

    # Si la página está en blanco (sólo contiene un salto de página), eliminar el salto de página
    if page_range.Text == "\f" and i in [16, 17]:
        page_range.Delete()
